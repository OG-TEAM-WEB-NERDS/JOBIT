...
export const jobsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getRecommendedJobs: builder.query({
      query: () => `search?query="software developer"&date_posted=3days&remote_jobs_only=true&employment_types=fulltime&rapidapi-key=${jobsApiKey}`,
    }),
    ...
    getFeaturedCompanies: builder.query({
      query: () => `search-filters?query="software developer"&rapidapi-key=${jobsApiKey}`,
    }),

    // Get jobs by company
    getJobsByCompany: builder.query({
      query: (employerId) => `search?query="software developer"&employer=${employerId}&rapidapi-key=${jobsApiKey}`,
    }),
  }),
});
